...
## ПРАВИЛО АРТ-ПАЙПЛАЙНА — 21.09.2026

**Не жертвувати силуетом заради кількості граней, доки профіль на СПРАВЖНЬОМУ телефоні не
покаже, що ми впираємось у геометрію.**

Підстава — власний замір, а не міркування. За один день примітиви в кадрі зросли з 95 414 до
240 006 (×2,5: другий ряд забудови, дев'ять будинків по 6000 граней, чотири хатини), і при
цьому:

    draw calls    230 → 206   (впали, бо все йде тими самими пачками MultiMesh)
    кадр          60 к/с, середнє 16,7 мс — без змін
    текстури      36,7 МБ — без змін

Перевірено й зворотне: house_terra_4 на 4800 гранях замість 6000 дає на ігровій відстані
**0,006% змінених пікселів** і 2 400 примітивів економії з 227 тисяч. Тобто міняти якість на
геометрію зараз немає за що.

Отже полігони можна витрачати там, де вони справді видні: силует даху, обрис труби, дверний
отвір, великі балки, краї на тлі неба, форми героїв. А поверхневу деталь тримає карта
нормалей — і тільки після того, як розгортка нормальна (див. «СТАН МОДЕЛЕЙ» і урок про
заповнення atlas'а в memory bank).

**Коли це правило перестає діяти.** Коли профіль на телефоні покаже час ВІДЕО більший за час
ЦП і при цьому кадр не тримається. До того — не чіпати.
