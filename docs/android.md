...
Ручки командного рядка для збірок заміру (телефону змінних середовища не передати, тож вони
йдуть рядком у пресеті експорту): `--level=N` — одразу в рівень, `--scale=0.8` — масштаб
рендера, `--strip=farcards,nofar,…` — те саме, що `STRIP=` на Маку (прапорці перелічено в
`Run3D.debug_strip`). Так дві збірки для живої розгортки різняться одним рядком.
