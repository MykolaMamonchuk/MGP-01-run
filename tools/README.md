...
| `prop_bake.py` | **Ретопологія із запіканням** (Blender headless). Робить із важкої моделі грубу ціль, розгортає її наново й запікає на неї поверхню оригіналу: колір → базова текстура, нахил → карта нормалей. Саме так дрібний рельєф (черепиця, дошки, цегла) переїздить із геометрії в текстуру. `--in --out --tris 2000 --size 1024 --box ШxВxГ --cage 0.03`. Перевірено на house_terra: 30 918 гр. / 34,4 МБ → 1 998 гр. / 0,45 МБ, вигляд збережено |
