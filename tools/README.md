...
Змінні: `STAGE` (`menu` типово / `level`), `LEVEL`, `FRAMES`, `OUT`.
...
**Відома пастка: проба бачить ВАШ збережений прогрес** (`user://`), тому стартовий екран
може бути різним — з чистим записом це заставка, з пройденими рівнями одразу мапа. Для
порівнянь «до/після» або міряйте один і той самий `STAGE=level`, або спершу зносьте
`~/Library/Application Support/Godot/app_userdata/Bizhy-Bizhy/`.
