...
PAL = {
    ...
    "straw_hi": "#FAD073", "straw": "#EDB040", "straw_edge": "#D7951F", "straw_line": "#B87812",
...
def materials(k):
    return {
        ...
        "knot": k.mat_gradient("knot", [(0.0, "knot_lo"), (1.0, "knot")], 1.3, 1.5, 0.4, 12.0),
...
def build(m, k):
    ...
    sphere("knot", m["knot"], (0.06, -hd + 0.12, PEAK + 0.12), 0.1, (1.3, 1.0, 0.75))
    for sgn in (-1, 1):
        cyl("beam_end%d" % sgn, m["beam"], (sgn * (hw - 0.02), -hd - 0.06, H + 0.05), 0.045, 0.16, rot=(math.pi / 2, 0, 0))
    cyl("beam_top", m["beam"], (0.0, -hd - 0.04, PEAK - 0.1), 0.045, 0.12, rot=(math.pi / 2, 0, 0))

    # Теракотовий димар ліворуч, зі сходинкою зверху.
    ch = box("chimney", m["brick"], (-0.3, 0.02, 1.18), (0.18, 0.18, 0.62))
    soften(ch, 0.02)
    lip = box("chimney_lip", m["brick"], (-0.3, 0.02, 1.47), (0.24, 0.24, 0.06))
    soften(lip, 0.015)
    lip2 = box("chimney_lip2", m["brick"], (-0.3, 0.02, 1.53), (0.22, 0.22, 0.05))
    soften(lip2, 0.015)
    ...
    bpy.ops.mesh.primitive_torus_add(major_radius=0.1, minor_radius=0.03, major_segments=ts,
                                     minor_segments=tr_, location=(0.0, fy, 0.9), rotation=(math.pi / 2, 0, 0))
    ring = bpy.context.active_object
    ring.name = "win_ring"
    ring.data.materials.append(m["ring"])
    cyl("win_glass", m["glass"], (0.0, fy + 0.012, 0.9), 0.1, 0.012, rot=(math.pi / 2, 0, 0))
    box("win_v", m["ring"], (0.0, fy - 0.006, 0.9), (0.018, 0.02, 0.18))
    box("win_h", m["ring"], (0.0, fy - 0.006, 0.9), (0.18, 0.02, 0.018))
    sill = box("win_sill", m["stone"], (0.0, fy - 0.04, 0.77), (0.26, 0.08, 0.06))
    soften(sill, 0.015, min(2, k.DET["bev"]))

    # Жовтий вигнутий козирок над дверима — товста дуга (та сама заготовка, що й рамки арок).
    arch_curve("awning", m["awning"], 0.34, 0.0, fy - 0.06, 0.075, 0.5)
    ...
    for sgn in (-1, 1):
        ...
        bpy.ops.mesh.primitive_torus_add(major_radius=0.08, minor_radius=0.026, major_segments=ts,
                                         minor_segments=tr_, location=(x, -0.02, 0.48), rotation=(0, math.pi / 2, 0))
        rr = bpy.context.active_object
        rr.name = "side_ring%d" % sgn
        rr.data.materials.append(m["ring"])
        cyl("side_glass%d" % sgn, m["glass"], (x - sgn * 0.01, -0.02, 0.48), 0.08, 0.012, rot=(0, math.pi / 2, 0))
    return {}
