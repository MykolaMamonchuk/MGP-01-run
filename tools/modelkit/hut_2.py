...
PAL = {
    # Рендер-замір світить зеленим тлом малюнка, а сам малюнок освітлений справа: стіни тут
    # на крок тепліші (менше зеленого), дах — яскравіший, інакше правий скат виходив оливковим.
    "wall_hi": "#E2D4A6", "wall": "#D8CA98", "wall_lo": "#CFC18E", "wall_line": "#C8BA88",
    "roof_hi": "#FFCC52", "roof": "#FFB82E", "roof_edge": "#F6A422", "roof_line": "#E0921A",
    "trim": "#3F6B34",
    "stone_hi": "#D0D2CC", "stone": "#BCBFB8", "stone_lo": "#A4A89E",
...
def build(m, k):
    W, D = 1.0, 0.86
    # Дах на малюнку займає більше половини висоти хати: щипець вищий (було 1,24).
    H, PEAK = 0.74, 1.36
    hw, hd = W * 0.5, D * 0.5
    ...
    # На малюнку димар широкий (~0,26 м), сидить на правому скаті ближче до гребеня, а його
    # рожева шапка — трохи НИЖЧЕ гребеня (раніше стирчав над дахом на чверть метра).
    ch = box("chimney", m["chimney"], (0.34, 0.04, 1.2), (0.26, 0.26, 0.56))
    soften(ch, 0.03)
    cp = box("chimney_cap", m["pink"], (0.34, 0.04, 1.49), (0.33, 0.33, 0.07))
    soften(cp, 0.025)
