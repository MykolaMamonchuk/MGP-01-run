...
def build(m, k):
    ...
    for side in (-1, 1):
        ...
        soften(r, 0.065)
        # зелена смуга під звисом на фасадному щипці
        tz = z_gable - 0.01
        tr = box("trim%d" % side, m["trim"], (side * (hw * 0.5 + 0.02), -hd - 0.005, tz), (run - 0.3, 0.02, 0.05),
    ...
    st = box("step", m["stone"], (0.0, fy - 0.09, 0.03), (0.4, 0.14, 0.06))
    soften(st, 0.02)
