...
МЕЖІ МІРИ (рецензія 26.09, чесно):
  • силует чутливий до рамки обрізання малюнка: зсув crop на 2-4% гойдає його на ±3-6 пунктів,
    тож число біля порогу 60-70% — не вирок, а сигнал подивитись оком;
  • сильна віньєтка (центр рядка світліший за краї більш ніж на ~45 RGB) лишається «об'єктом»;
  • стіна, що відрізняється від тла на < 10 RGB з розмитим краєм, може «протекти» в тло —
    силует тоді занижено (помилка в безпечний бік);
  • ЗАКРИТИХ дір маска малюнка не бачить (заливка тла в них не заходить), тому й у моделі
    закриті дірки заповнюються: модель-кільце проти малюнка-диска дає ~90%. Різницю тоді несе
    лише «колір на місці» (у «разом» не входить) — дивись на нього, якщо в моделі є отвори.
...
def main():
    ...
    # Палітра (межі міри — у докстрінгу модуля):
    ...
    # Центри — ДЕТЕРМІНОВАНО (найдальші точки від уже вибраних, по обох вибірках разом), а
    # близькі (< 20 RGB) — зливаються. Із випадковим стартом k-середніх той самий колір
    # розкидало по двох майже однакових кошиках, і малюнок проти самого себе в JPEG давав
    # палітру 44%, а на справжніх малюнках число гуляло від зерна на ±4 (рецензія 26.09).
    pool = sr + so
    cent = [tuple(sum(c[j] for c in pool) / len(pool) for j in range(3))]
    while len(cent) < 8:
        far = max(pool, key=lambda c: min(d2(c, e) for e in cent))
        if min(d2(far, e) for e in cent) < 20 * 20:
            break
        cent.append(far)
    for _ in range(12):
        ...
        merged = []
        for c in cent:
            if all(d2(c, e) >= 20 * 20 for e in merged):
                merged.append(c)
        cent = merged
    def dist(src):
        hst = [0] * len(cent)
