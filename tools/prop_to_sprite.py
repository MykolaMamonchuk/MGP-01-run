...
def shrink(png, colors):
    ...
    # Ширина ОДНОГО кадру, а не всього файлу. Для спрайта з кількох боків (атлас у рядок)
    # файл ширший у стільки ж разів, скільки в ньому кадрів, і метраж виходив би в стільки
    # ж разів меншим — кущ став би завбільшки з монету. Висота завжди одного кадру, тож
    # квадратний знімок дає число кадрів діленням.
    full_px = im.size[1]
