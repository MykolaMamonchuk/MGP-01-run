...
func _ready() -> void:
	...
	# АВТОРСЬКИЙ РІВЕНЬ. Знімок без LEVEL показує лише процедурне узбіччя; з LEVEL до нього
	# додаються маркери рівня — саме те, що побачить гравець.
	#
	# ТУТ БУЛО НАПИСАНО НАВПАКИ, і це вводило в оману: мовляв, «коли в рівня є авторський
	# декор, Track малює ТІЛЬКИ його, а процедурний декоратор не виконується взагалі».
	# Так було колись, і саме через це рівень після першого чанку ставав порожнім: 30
	# маркерів декору на 305 метрів — один предмет на десять метрів. Тепер авторські
	# маркери кладуться ПОВЕРХ звичайного оздоблення (див. коментар у track.gd,
	# _decorate()), і обидва джерела працюють разом.
	...
	if OS.get_environment("VIEW") == "top":
		...
		# інакше порівняння з референсом міряло б ракурс, а не рівень. near/far — теж РІВНО
		# як у грі (діагноз миготіння: near=0.05 при типовому far~4000 з'їдав майже всю
		# точність буфера глибини на далеких предметах); інакше цей знімок міряв би не те
		# мерехтіння, яке бачить гравець.
...
func _process(_delta: float) -> void:
	...
	# СЕРІЯ кадрів рівного руху. Задум був такий: рух рівний і монотонний, а миготіння — ні,
	# тож піксель, що блимає, ходить угору-вниз-угору, а край предмета, який наїжджає,
	# змінюється в один бік; отже досить порахувати ЗМІНИ НАПРЯМКУ.
	#
	# ОБЕРЕЖНО, ЗАДУМ ХИБНИЙ САМЕ ДЛЯ РУХУ, і це перевірено. Коли повз піксель проїжджає
	# ПОВТОРНИЙ візерунок — поручні берега, дошки містка, обручі бочки, — той піксель
	# чесно бачить поперемінно дерево й просвіт. Напрямок міняється щокадру, і лічильник
	# зараховує це як миготіння, хоча блимає лише частокіл, що їде повз. Заміряно: MSAA 8×
	# замість 4× не змінює нічого (14,12% → 14,10%), вимкнені тіні не змінюють нічого
	# (14,41%), вимкнена деталізація не змінює нічого (14,11%), вимкнене «дихання» декору
	# не змінює нічого (14,15%) — а прибрані поручні берега знімають одразу 21% (11,09%).
	# Тобто міра рахує КІЛЬКІСТЬ ДРІБНОГО ВІЗЕРУНКА В КАДРІ, а не ваду.
	#
	# Для НЕРУХОМОЇ камери (STEP=0) лічильник чесний і корисний: там рухатись нічому.
	# Для руху потрібне вирівнювання кадрів за відомим зсувом траси — це ще не зроблено.
	#
	#     WORLD=meadow LEVEL=1 ADVANCE=120 STEP=0.1 FRAMES=9 SERIES=/tmp/f godot res://src/debug/track_shot.tscn
	var series := OS.get_environment("SERIES")
	if series != "":
		var n := int(OS.get_environment("FRAMES")) if OS.get_environment("FRAMES") != "" else 8
		var d := float(OS.get_environment("STEP")) if OS.get_environment("STEP") != "" else 0.1
		for k in range(maxi(2, n)):
			if k > 0:
				_track.advance(d)
				await RenderingServer.frame_post_draw
			_save("%s%02d.png" % [series, k])
		print("серія з %d кадрів, крок %.2f м: %s00.png …" % [maxi(2, n), d, series])
		get_tree().quit()
		return
