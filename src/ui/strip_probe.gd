...
## ЗАГАЛЬНИЙ набір; під разовий дослід правлять і повертають назад разом із результатом.
##
## УВАГА: «roadbase» тепер нічого не змінює, бо основу прибрано за замовчуванням (стиль
## «grid»). Якщо знадобиться міряти її знову — спершу повернути стиль «base», інакше проба
## покаже нуль і з цього легко народиться хибний висновок.
const VARIANTS := [
	...
	{"назва": "без дрібниці декору", "flags": ["props"]},
