...
## ДОСЛІД 25.09: ЧИ ЗРОБИЛО ОСВІТЛЕННЯ НА ВЕРШИНУ НАДЩІЛЬНІ ХАТИ ДОРОГИМИ.
##
## Розклад лугу дав забудові ~4 мс на робочому масштабі 0,80 — кандидат. Агент пов'язав це з
## вершинами: `house_terra_*` мають 8-11 тисяч вершин кожна. Але 22.09 саме цю модель
## тестували контрольовано (8360 -> 2100 -> 500 вершин) — і нуль. Різниця між тоді й тепер
## одна, і ми її зробили самі: декор тепер світиться НА ВЕРШИНУ, тобто світло рахується на
## кожну з ~213 тисяч вершин цих хат.
##
## Тест без ризику для вигляду: повернути на піксель лише ці хати. Якщо кадр прискориться —
## винне освітлення на вершину на щільних сітках, і лікується це правилом, а не LOD
## (який замовник уже раз забракував через черепицю).
## Очікуваний структурний лічильник: terrapixel/allpixel — БЕЗ ЗМІН (правка матеріалу);
## buildings — виклики й примітиви ВНИЗ.
const VARIANTS := [
	{"назва": "усе як є", "flags": []},
	{"назва": "лише house_terra на піксель", "flags": ["terrapixel"]},
	{"назва": "увесь декор на піксель", "flags": ["allpixel"]},
	{"назва": "без забудови (стеля)", "flags": ["buildings"]},
