...
## ДОСЛІД 25.09: ВІДСІВ КАНДИДАТІВ. Однаковий набір для ЧОТИРЬОХ найгірших світів (луг,
## місто, хмаринки, пляж) — щоб числа були порівнянні між світами, а не лише всередині.
##
## Питання не «що коштує найбільше взагалі», а конкретне: ЩО САМЕ тримає цей світ вище
## 33,3 мс (тобто вище 30 к/с). Усе, чий теоретичний максимум менший за 2 мс, відсіюємо
## одразу: жива розгортка таку правку підтвердити не може (шум 0,6-1,4 мс на рівень плюс
## зсув прогону ~0,9 мс), тож оптимізувати там означало б оптимізувати шум.
##
## Очікуваний структурний лічильник: УСІ варіанти прибирають геометрію, отже виклики й
## примітиви мусять піти ВНИЗ у кожному. Якщо десь не пішли — міряємо не те.
const VARIANTS := [
	{"назва": "усе як є", "flags": []},
	{"назва": "без декору зовсім", "flags": ["decor"]},
	{"назва": "без забудови", "flags": ["buildings"]},
	{"назва": "без полотен дороги", "flags": ["track"]},
