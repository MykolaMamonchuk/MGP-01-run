...
## ДОСЛІД 25.09: КАНАЛИ ЛУГУ. Після забудови це найдорожче, що там лишилось: розклад дав
## 4,6 мс. Вода в каналах уже на дешевому шейдері без освітлення, тож ціна не в математиці,
## а в ПРОЗОРОСТІ — прозорий прохід не пише глибину, і все за ним теж малюється.
##
## Непрозору воду ми свого часу не взяли: вона дає 2,8 мс, але забирає прибережну піну
## навколо перешкод у воді. У КАНАЛАХ перешкод немає ЖОДНОЇ, тож там ця ціна не платиться —
## і море можна лишити прозорим. Саме тому прапорець діє лише на канали.
## Очікуваний структурний лічильник: `canal` міняє виклики ВНИЗ, `canalopaque` — без змін
## (та сама геометрія, інший шейдер).
const VARIANTS := [
	{"назва": "як у грі (прозорі канали)", "flags": []},
	{"назва": "канали НЕПРОЗОРІ", "flags": ["canalopaque"]},
	{"назва": "каналів немає зовсім", "flags": ["canal"]},
	{"назва": "контроль: як у грі", "flags": []},
]
