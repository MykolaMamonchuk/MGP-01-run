...
## без нього «стало гірше» не відрізнити від «телефон нагрівся» (див. docs/MEMORY.md).
##
## Тут стоїть ЗАГАЛЬНИЙ набір — той, яким здобуто чинне ранжування систем. Під разовий
## дослід список правлять, але повертають назад разом із результатом: інструмент, у якому
## лишилась чужа разова конфігурація, наступного разу тихо поміряє не те. Один раз він уже
## лишився з єдиною точкою, двома варіантами й без контролю.
const VARIANTS := [
