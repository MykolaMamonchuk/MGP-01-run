...
## ДОСЛІД 24.09: ОСВІТЛЕННЯ ДЕКОРУ НА ВЕРШИНУ. Старий запис каже, що воно ГІРШЕ за на
## піксель на 47 одиниць — але те число знято до всіх виправлень діагностики, зокрема до
## того, як знайшлось аліасування ключів матеріалів (воно давало 24% похибки). Дерево має
## 42 вершини й 24 трикутники, тож «на вершину дорожче за на піксель» контрінтуїтивне
## настільки, що без повторної перевірки його приймати не можна.
const VARIANTS := [
	{"назва": "усе як є (на піксель)", "flags": []},
	{"назва": "декор НА ВЕРШИНУ", "flags": ["pervertex"]},
	{"назва": "декор без освітлення", "flags": ["unshaded"]},
