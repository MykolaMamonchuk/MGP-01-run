...
## ДОСЛІД 25.09: ХАТА СПРАЙТОМ З 8 БОКІВ проти запеченої оболонки.
##
## Оболонка на 600 трикутників уже дала -3,1 мс. Спрайт — 4 вершини замість 1800, тож за
## нахилом ~0,47 мс на тисячу вершин очікуємо ще ~0,8 мс. Але спрайт має прозорі краї
## (alpha scissor), а на тайловому GPU такі пікселі вимикають ранню перевірку глибини — і
## хати займають велику частку екрана. Тож він може вийти й ДОРОЖЧИМ за оболонку. Міряємо.
## Очікуваний структурний лічильник: примітиви падають orig > proxy600 > sprite, виклики
## незмінні.
const VARIANTS := [
	{"назва": "усі хати = h6 оригінал", "flags": ["terramesh_orig"]},
	{"назва": "усі хати = proxy600", "flags": ["terramesh_proxy600"]},
	{"назва": "усі хати = спрайт 8 боків", "flags": ["terramesh_sprite"]},
