...
const POINTS: Array[float] = [180.0]

## ДОСЛІД 24.09: РОЗКЛАД ДЕКОРУ на теперішній збірці. Старе «30 мс» історичне — воно знято
## до вимкнення тіней і заміни туману, і перезамір уже дав 4,5 мс. Тепер питання вужче: з
## чого саме ці мілісекунди складаються — з пікселів, з освітлення чи з текстур.
const VARIANTS := [
	{"назва": "усе як є", "flags": []},
	{"назва": "декору нема", "flags": ["decor"]},
	{"назва": "декор без освітлення", "flags": ["unshaded"]},
	{"назва": "декор одним матеріалом", "flags": ["onemat"]},
	{"назва": "декор без текстур", "flags": ["onetex"]},
