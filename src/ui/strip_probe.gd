...
## ДОСЛІД 24.09: МІСТО, сьома серія — перевірка зустрічного результату.
##
## Розгортка 17 рівнів (три прогони, контроль зійшовся на 0,6%) сказала несподіване: на
## лузі правка з випроміненням виграє 6,8 мс, а в МІСТІ програє 4,1 — тобто там вона не
## економить, а коштує. Розгортка — інструмент «де погано», а не «скільки коштує», тож
## перевіряємо те саме точним: заморожена точка, пари, контроль.
## Очікуваний структурний лічильник: виклики й примітиви БЕЗ ЗМІН (правка матеріалу).
const VARIANTS := [
	{"назва": "як у грі (випромінення)", "flags": []},
	{"назва": "без правки (навкол. світло)", "flags": ["noemis"]},
	{"назва": "як у грі — контроль", "flags": []},
	{"назва": "без правки — контроль", "flags": ["noemis"]},
]
