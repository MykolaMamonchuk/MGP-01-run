...
## МАТРИЦЯ ПРОФІЛІВ на справжньому рівні. Розклад уже зроблено (див. коміт d13e19b):
## фонова забудова — головний чинник, придорожня дрібниця майже нічого не важить, тіні
## важливі, а виклики й примітиви час не пояснюють. Тому далі крутимо не по одній ручці, а
## комплектом: масштаб + забудова + тіні разом, бо кожна поодинці впирається в іншу.
## «Полегшена забудова» тут — кожен другий шар стіни (прапорець half): чесне наближення
## до меншої щільності, а не готовий продуктовий варіант.
const STEPS := [
