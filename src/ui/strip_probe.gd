...
## ДОСЛІД 23.09: одна точка, бо варіантів сім. Memory bank: на восьми варіантах дрейф від
## нагрівання сягав 9,6%, на чотирьох-п'яти — 0,2-0,5%. Друга точка подвоїла б серію, тому
## спершу одна з контролем, і лише якщо контроль зійдеться — повтор на другій.
const POINTS: Array[float] = [60.0]   # загальне: [60.0, 180.0]
...
## ДОСЛІД 23.09: розклад «полотен дороги» (~20 мс) по окремих шарах. Основа (roadbase) і
## плитка (roadtiles) вже перевірені й безкоштовні, тож беремо решту п'ять. Загальний набір
## лежить нижче закоментований — повернути разом із результатом.
## ДОСЛІД 23.09, друга серія: СКІЛЬКИ РАЗІВ малюється земля під героєм.
## Перша серія показала підпис накладання: жоден шар окремо не коштує (разом 9,8), а всі
## разом — 22,3. Прибираєш один — пікселі малює наступний під ним. Тут знімаємо шари
## ПАРАМИ й цілком, щоб побачити, скільки їх насправді лежить одне на одному.
const VARIANTS := [
	...
	{"назва": "полотен нема зовсім", "flags": ["roadbase", "roadtiles", "mm_side", "mm_cliff", "mm_seam", "mm_edge", "mm_clouds"]},
	{"назва": "контроль: усе як є", "flags": []},
]

## ЗАГАЛЬНИЙ набір — повернути після досліду:
##	{"назва": "усе як є", "flags": []},
