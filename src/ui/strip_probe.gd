...
## ДОСЛІД 25.09: ОСВІТЛЕННЯ ПОЛОТНА ДОРОГИ — стеля єдиного великого кандидата.
##
## Відсів показав: дорога коштує 13-15,5 мс на лузі, хмаринках і пляжі, і це ЗАПОВНЕННЯ, не
## геометрія. Гіпотеза: полотно пласке, видно майже лише верхню грань (нормаль угору), тіні
## в «Плавно» вимкнено — отже освітлення рахує на кожному пікселі одне й те саме число.
##
## Міряємо стелю (`roadunlit`) і те, скільки з неї дає сама гілка навколишнього світла
## (`roadnoamb`) — на декорі це було 86%, і саме її вдалось віддати випроміненню без втрат.
## Очікуваний структурний лічильник: виклики й примітиви БЕЗ ЗМІН у всіх варіантах — це
## правки матеріалу. Для порівняння лишаємо «без полотен», де вони мусять упасти.
const VARIANTS := [
	{"назва": "усе як є", "flags": []},
	{"назва": "дорога без освітлення", "flags": ["roadunlit"]},
	{"назва": "дорога без навкол. світла", "flags": ["roadnoamb"]},
	{"назва": "дорога на вершину", "flags": ["roadvertex"]},
	{"назва": "без полотен зовсім (мірка)", "flags": ["track"]},
