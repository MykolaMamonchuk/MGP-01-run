...
## ОДНА ТОЧКА для цього досліду. Прогін на двох точках дав на 60 м розсип (175, 156, 133,
## 172, 173, контроль 135) при самоузгоджених числах на 180 м. Перевірено ділом, що це НЕ
## залипання прапорців: і `unshaded`, і `onemat` відновлюються чисто. Отже ненадійна сама
## точка, і брати з неї числа не можна. Загальне: [60.0, 180.0]
const POINTS: Array[float] = [60.0, 180.0]

## ДОСЛІД 25.09: СИТКА ХАТ — ЗАПЕЧЕНА ОБОЛОНКА І ФАСАДНИЙ ЗРІЗ.
##
## Одна модель (house_terra_6, 5998 трикутників) ставиться замість УСІХ хат house_terra* у
## кадрі — інакше ефект однієї моделі (3 екземпляри з ~22) нижчий за поріг вимірювання.
## Опора — та сама house_terra_6 в оригіналі, щоб порівнювати сітку з сіткою.
##   proxy1200 — нова оболонка 1200 трикутників, вигляд оригіналу запечено в текстуру;
##   proxy600  — те саме на 600 (текстура запеклась із браком, але ціна від неї не залежить);
##   facadeA   — оригінал без граней, невидимих з дороги (-21% трикутників);
##   facadeB   — жорсткіший зріз (-36%).
## 22.09 саме ця модель, спрощена до ~2100 і ~500 вершин, дала НУЛЬ. Відтоді змінились умови:
## тіні вимкнено, світло на вершину. Цей дослід відповідає, чи це щось змінило.
## Очікуваний структурний лічильник: примітиви мусять ПАДАТИ від опори до proxy600, а
## виклики — лишатись незмінними (шарів стільки ж, змінюється лише сітка в них).
const VARIANTS := [
	{"назва": "усі хати = h6 оригінал", "flags": ["terramesh_orig"]},
	{"назва": "усі хати = proxy1200", "flags": ["terramesh_proxy1200"]},
	{"назва": "усі хати = proxy600", "flags": ["terramesh_proxy600"]},
	{"назва": "усі хати = facadeA", "flags": ["terramesh_facadeA"]},
	{"назва": "усі хати = facadeB", "flags": ["terramesh_facadeB"]},
	{"назва": "контроль: h6 оригінал", "flags": ["terramesh_orig"]},
]
