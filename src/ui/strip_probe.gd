...
## ДОСЛІД 25.09: МІСТО — ЩО В НЬОМУ НЕ ЗАЛЕЖИТЬ ВІД МАСШТАБУ РЕНДЕРА.
##
## Місто — найгірший світ (26 к/с), і на масштаб рендера воно не реагує (нахил −0,20 мс на
## прольоті 0,80->0,83, тоді як луг +1,33, пляж +1,90). Прибрати забудову — кадр ГІРШИЙ на
## 10-12 мс: стіни затуляють дорогий фон. Отже заповнення тут не головне, і шукати треба
## серед того, чого масштаб рендера НЕ зачіпає. Масштаб зменшує лише буфер 3D, а корекція
## кольору, інтерфейс і частинки йдуть на ПОВНІЙ роздільності екрана.
##   adjust      — корекція кольору (насиченість 1,15) — повноекранний прохід;
##   ui          — увесь інтерфейс;
##   particles   — частинки;
##   noobstacles — перешкоди й предмети на доріжках.
## Прапорець із РІВНОЮ дельтою на всіх сходах масштабу — ціна не заповнення.
## Очікуваний структурний лічильник: ui/adjust — виклики 3D БЕЗ ЗМІН; particles/noobstacles
## — виклики ВНИЗ.
const VARIANTS := [
	{"назва": "усе як є", "flags": []},
	{"назва": "без корекції кольору", "flags": ["adjust"]},
	{"назва": "без інтерфейсу", "flags": ["ui"]},
	{"назва": "без частинок", "flags": ["particles"]},
	{"назва": "без перешкод", "flags": ["noobstacles"]},
