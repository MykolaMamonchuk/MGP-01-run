...
## ДРАБИНА МАСШТАБУ РЕНДЕРА — не лише оптимізація, а ЗОНД: вона відповідає на питання,
## наскільки ми зараз упираємось у пікселі. Якщо 0,87 зніме багато — решта ціни теж у
## заповненні; якщо майже нічого — треба йти в декор.
## Орієнтир ззовні: гра про пліт малює в 1280x720 і розтягує до 1465x720, тобто 0,87.
const STRIP_SETS := [
	{"назва": "масштаб 1.00", "flags": ["scale100"], "water": -1},
	{"назва": "масштаб 0.90", "flags": ["scale90"], "water": -1},
	{"назва": "масштаб 0.87", "flags": ["scale87"], "water": -1},
	{"назва": "масштаб 0.80", "flags": ["scale80"], "water": -1},
	{"назва": "масштаб 0.70", "flags": ["scale70"], "water": -1},
]
...
## Підписи називають стилі тим, чим вони є, а не тим, чим були: типовим тепер є «grid»
## (прибрана основа дає 4,9 мс), а «base» — це старий вигляд із суцільною основою під
## плиткою. Перший підпис колись казав «ЗАРАЗ», і після зміни типового це збивало.
const ROAD_LABELS := {
	"base": "дорога: стара (з основою)",
	"flat": "дорога: гладка",
	"grid": "дорога: ЗАРАЗ (з лініями)",
}
