...
##
## Маркери лежать у ЛОКАЛЬНИХ метрах — від початку самого чанка. Спершу вони були в
## абсолютних метрах рівня (chunk_05 починався на z = −750), і відкритий у редакторі чанк
## через це виглядав порожнім: усе його добро стояло за сотні метрів від початку координат.
##
## Де саме стоїть чанк, сама сцена НЕ ЗНАЄ і знати не повинна: зсув передає той, хто її
## ставить (LevelChunkLoader, а згодом — збирач зі списку цеглинок,
## docs/tasks/reusable-chunks.md). Саме це й робить чанк придатним до перевикористання:
## та сама цеглинка може стояти на 150-му метрі рівня 1 і на 900-му рівня 17.
##
