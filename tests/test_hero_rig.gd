...
func test_unicorn_hero_data() -> void:
	...
	# й саджаємо нижче, інакше очі-«блоки» вилазять аж до вух/рога. Голова майже кубічна
	# (0.56×0.58×0.60 м) — це НЕ довга кінська морда, тож очі лишаються СПЕРЕДУ, а не
	# збоку (`layout: "side"` тут раніше стояв від старої моделі й ховав намальовані очі
	# за вухами/гривою — самі очі на моделі лишались непофарбованими западинами)
	var f := HeroRig.face_of(odn)
	assert_almost_eq(float(f["scale"]), 0.35, 0.0001)
	assert_almost_eq(float(f["y"]), 0.45, 0.0001)
	assert_eq(String(f["layout"]), "front", "компактна голова — очі спереду, як у решти героїв")
