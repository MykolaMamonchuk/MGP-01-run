...
func test_carousel_is_not_built_until_opened() -> void:
	...
	assert_eq(_hs.ids.size(), 7, "але список героїв уже прочитано — він потрібен геймплею")
...
func test_opening_builds_carousel_once() -> void:
	...
	assert_eq(_hs._previews.size(), 7, "усі герої на місці")
	...
	assert_eq(_hs._previews.size(), 7)
