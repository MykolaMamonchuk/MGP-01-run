...
func test_carousel_is_not_built_until_opened() -> void:
	# важку карусель (7 звірят із подіумами; пуфи — legacy) не будуємо на старті гри — OPT-04
	assert_not_null(_hs, "HeroSelect у сцені")
	assert_eq(_hs.get_child_count(), 0, "до відкриття карусель не займає нічого")
	assert_eq(_hs.ids.size(), 9, "але список героїв уже прочитано — він потрібен геймплею")
...
func test_opening_builds_carousel_once() -> void:
	...
	assert_eq(_hs._previews.size(), 9, "усі герої на місці")
	...
	assert_eq(_hs._previews.size(), 9)
