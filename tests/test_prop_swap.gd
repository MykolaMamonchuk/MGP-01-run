...
func test_world_one_obstacles_name_their_target_model() -> void:
	...
	# (beehive із anim "roll"), просто нею ніхто не користувався. 11 → 12 (26.09.2026):
	# goose_fly — гуска, що ЛЕТИТЬ на висоті голови (пригнись), зі скелетною анімацією.
	# 12 → 13 (26.09.2026): goose_run — пасеться за краєм і перебігає у свою доріжку.
	assert_eq(with_target, 13, "перешкоди першої черги з названою цільовою моделлю")
